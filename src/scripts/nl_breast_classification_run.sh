...
source "/restricted/projectnb/batmanlab/shawn24/PhD/Breast-CLIP-downstream/src/scripts/configs/nl_breast_configs.sh"

NL_Breast_LABELS=("cancer")
NL_Breast_FRACS=("1.0" "0.8" "0.5" "0.25" "0.1")
NL_Breast_ARCHS=("breast_clip_det_b5_period_n_lp" "breast_clip_det_b5_period_n_ft")



for LABEL in "${NL_Breast_LABELS[@]}"; do
  for ARCH in "${NL_Breast_ARCHS[@]}"; do
    for FRAC in "${NL_Breast_FRACS[@]}"; do
      ...
      echo "Submitting NLBreast: $jobname"
      qsub -j y -N "$jobname" \
